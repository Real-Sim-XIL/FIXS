...
#if !defined(CM4SLDS) || defined(_DSRT) || defined(_DSRTLX)
...
#endif /* #if !defined(CM4SLDS) || defined(_DSRT) || defined(_DSRTLX) */
...
MainThread_BeginCycle (unsigned long long CycleNo64)
{
    ...
    if (SimCore.InfoParamsUpdated) {
	InfoParam_SwitchValues();
	SimCore.InfoParamsUpdated = 0;
    }
...
static pthread_t IPGRT_BgThread;
static int       IPGRT_BgRun = 0;
...
{
#if defined(_DSRT)
    pthread_setname_np(pthread_self(), "IPGRT");
#endif
    while (IPGRT_BgRun)
	IPGRT_Poll();	/* no busy loop, sleeps by calling select() */
...
ipgrt_init (void)
{
    /* Rendez-Vous with host */
    if (IPGRT_RendezVous()) {
	fprintf(stderr, "ERROR\tRendez-Vous with CarMaker host failed\n");
	return -1;
    }
#if defined(_DSRT) || defined(_DSRTLX)
    IPGRT_BgRun = 1;
    if (pthread_create(&IPGRT_BgThread, NULL, ipgrt_bg, NULL) != 0) {
	fprintf(stderr, "ERROR\tFailed to create IPGRT background thread\n");
	IPGRT_BgRun = 0;
	return -1;
    }
#  endif
...
{
#  if defined(_DSRT) || defined(_DSRTLX)
    if (pthread_kill(IPGRT_BgThread, 0) != ESRCH) {
	IPGRT_BgRun = 0;
	pthread_join(IPGRT_BgThread, NULL);
    }
#  endif
    IPGRT_Exit();
}
...
static int
CarMaker_Init (int argc, char **argv)
{
    int nError;
    ...
    if (MainThread_Init() < 0 && !SimCore.OnlyOneSimulation) {
	LogErrF(EC_Init, "MainThread_Init() failed");
	SimCore_MainThreadEmergency();
	return -1;
    }
    return 0;
...
setup_env (void)
{
    ...
    if (strlen(CM_CmdEnv) > 0)
	env = CM_SplitCmdEnv(CM_CmdEnv, NULL);
    if ((e = env) != NULL) {
	while (*e != NULL)
	    putenv(*e++);
	free(env);
...
static void
CarMaker_Init_Helper (void)
{
    char **argv;
    int    argc;

#  if defined(_DSRT) || defined(_DSRTLX)
    IPGRT_SetupErrorHandling();
#  endif

    char *appname = "CarMaker.x86";
    if (strlen(CM_AppName) > 0)
	AppStartInfo.App_Version = CM_AppName;
    if (strlen(CM_CmdLine) > 0) {
	argv = CM_SplitCmdLine(CM_CmdLine, appname, &argc);
    } else {
	argv = IPGRT_Argv;
	argc = IPGRT_Argc;
    }

    if (CarMaker_Init(argc, argv) == 0)
	CM_State = CM_InitOk;
    else
	CM_State = CM_InitFailed;
}


void
CarMaker_Startup (void)
{
    if (CM_State > CM_Uninitialized)
	return;
    setup_env();
    if (ipgrt_init())
	exit(EXIT_FAILURE);
    ...
    CM_State = CM_Init;
    CarMaker_Init_Helper();

    if (CM_State != CM_InitOk) {
	printf ("CarMaker_Init() failed\n");
	ipgrt_exit();
    }
}
...
static void
CarMaker_Exit_Helper (void)
{
    if (CarMaker_Exit() == 0)
	CM_State = CM_ExitOk;
    else
	CM_State = CM_ExitFailed;
}


void
CarMaker_Cleanup (void)
{
    CM_State = CM_Exit;
    CarMaker_Exit_Helper();

    if (CM_State != CM_ExitOk)
	printf ("CarMaker_Exit() failed\n");
}
#  endif /* !CM4SLDS || _DSRT || _DSRTLX */
