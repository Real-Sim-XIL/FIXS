...
#if defined(WITH_VISSIMIF)
#  include <VissimIF_CM.h>
#endif
...
#if defined(WITH_VISSIMIF)
    VissimIF_CM_TestRun_Start_atBegin();
#endif
    for (i=0; i<N_USEROUTPUT; i++)
	User.Out[i] = 0.0;
...
User_TestRun_Start_atEnd (void)
{
    int rv = 0;
#if defined(WITH_VISSIMIF)
    rv = VissimIF_CM_TestRun_Start_atEnd();
#endif
...
User_TestRun_RampUp (double dt)
{
    int IsReady = 1;
#if defined(WITH_VISSIMIF)
    IsReady = VissimIF_CM_RampUp(IsReady);
#endif
...
#if defined(WITH_VISSIMIF)
    VissimIF_CM_Check_IsIdle(IsIdle);
#endif
...
#endif
#if defined(WITH_VISSIMIF)
    if (VissimIF_CM_ApoMsg_Eval(Ch, Msg, len, who) == 0)
        return 0;
#endif
    return -1;
}
