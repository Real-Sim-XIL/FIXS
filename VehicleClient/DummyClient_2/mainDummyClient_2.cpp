...
const vector <int> serverPort = { 421 };
...
//vector <string> egoIdVec = { "vehicle_1" };
vector <string> egoIdVec = { "vehicle_1" };
...
string debugLogName = "../../MatlabCodes/timing_2.txt";
...
int main(int argc, char* argv[]) {
	...
	if (Config_c.getConfig("..\\..\\RealSimRelease\\config_SUMO_ACM.yaml") < 0) {
		return -1;
	}
	...
	while (1) {
		...
		if (Msg_c.VehDataRecv_um.find(egoIdVec[0]) != Msg_c.VehDataRecv_um.end()) {
			...
			//float speedmod = (float)(14 + 1 * sin(simTimeRecv));
			float speedmod = speedDes + (float)(((float)rand() / (RAND_MAX))-0.5)/10;
			VehDataSend["speedDesired"] = speedmod;
		}
		else {
			...
			if (Msg_c.VehicleMessageField_set.find("speedDesired") != Msg_c.VehicleMessageField_set.end()) {
				VehDataSend["speedDesired"] = (float)0;
			}
