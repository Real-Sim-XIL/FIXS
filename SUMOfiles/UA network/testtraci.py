        ...
        if len(vehId):
            ...
            for iV in range(0,len(vehId)):
                ...
                if idStr == egoIdStr:
                    ...
                    distanceTravel = traci.vehicle.getDistance(idStr)
                    x,y,z = traci.vehicle.getPosition3D(idStr)
                    ...
                    heading = traci.vehicle.getAngle(idStr)


                    xnew = x+0.1*curSpeed*math.sin(math.radians(heading))
                    ynew = y+0.1*curSpeed*math.cos(math.radians(heading))

                    link = traci.vehicle.getRoadID(idStr)
                    lane = traci.vehicle.getLaneID(idStr)
                    lanePosition = traci.vehicle.getLanePosition(idStr)
                    
                    #traci.vehicle.moveToXY(idStr,"",-1,, )

                    # traci.vehicle.setSpeedMode(idStr, 0)
                    # traci.vehicle.setSpeed(idStr, 15.7+math.sin(simTime))
                    # traci.vehicle.setSpeedMode(idStr, 32)
                    # traci.vehicle.setPreviousSpeed(idStr, curSpeed+0.1)
                    
                    print('simTime {}, speed {}, acceleration {}, distance travel {}, x {}, y {}, z {}'.format(simTime, curSpeed, curAccel, distanceTravel, x, y, z))
                    # print('\tmove to x {}, y {}'.format(xnew, ynew))
                    # print('\tlink {}, lane {}, lanePosition {}'.format(link, lane, lanePosition))
