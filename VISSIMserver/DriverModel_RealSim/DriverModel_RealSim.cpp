...
long vehPreferredRelLane = 0;
long vehUsePreferredLane = 0;
	...
	char* string_value)
{
	...
	case DRIVER_DATA_VEH_ID:
		/* long:   vehicle number */
		VissimVehDataCommon.id = to_string(long_value);
		DebugVar.id = long_value;

		return 1;
	case DRIVER_DATA_VEH_LANE:
		/* long:   current lane number (rightmost = 1) */
		VissimVehDataCommon.laneId = (int)long_value;
		return 1;
	...
	case DRIVER_DATA_VEH_TYPE:
		VissimVehDataCommon.type = to_string(long_value);
		return 1;
	case DRIVER_DATA_VEH_COLOR:
		VissimVehDataCommon.color = (uint32_t)long_value;
		return 1;
	case DRIVER_DATA_VEH_CURRENT_LINK:
		VissimVehDataCommon.linkId = to_string(long_value);
	...
	case DRIVER_DATA_VEH_NEXT_LINKS:
		...
		VehicleDataAuxiliary.nextLink_v.push_back(to_string(long_value));

		VissimVehDataCommon.linkIdNext = to_string(long_value);
		return 1;
	...
	case DRIVER_DATA_VEH_INTAC_STATE:
		...
		DebugVar.intacState = long_value;
		return 1;
	case DRIVER_DATA_VEH_INTAC_TARGET_TYPE:
		...
		DebugVar.intactTargetType = long_value;
		return 1;
	...
	case DRIVER_DATA_NVEH_ID:
		...
		*/
		...
		if (index1 == 0 && index2 == 1) {
			if (long_value > 0) {
				VissimVehDataCommon.precedingVehicleId = to_string(long_value);
	...
	case DRIVER_DATA_SIGNAL_STATE:
		...
		if (NEED_SIGNAL) {
			...
			//f << "signalLightId " << index1 << " signalLightHeadId " << index2 << " signalLightColor " << long_value << endl;
			//f.close();

			VissimVehDataCommon.signalLightId = to_string(index1);
			VissimVehDataCommon.signalLightHeadId = index2;
			VissimVehDataCommon.signalLightColor = long_value;
		}
		return 1;
	...
	case DRIVER_DATA_ACTIVE_LANE_CHANGE:
		...
		VissimVehDataCommon.activeLaneChange = long_value;
		return 1;
	case DRIVER_DATA_REL_TARGET_LANE:
		if (long_value == 0 && (active_lane_change != 0 || abs(desired_lane_angle) > 1e-5) ) {
	...
	char** string_value)
{
	...
	case DRIVER_DATA_VEH_DESIRED_VELOCITY:
		*double_value = VissimVehDataCommon.speedDesired;
		...
		return 1;
	case DRIVER_DATA_VEH_COLOR:
		*long_value = VissimVehDataCommon.color;
		return 1;
	case DRIVER_DATA_VEH_UDA:
	...
	case DRIVER_DATA_ACTIVE_LANE_CHANGE:
		...
		//}
		*long_value = VissimVehDataCommon.activeLaneChange;
	...
	case DRIVER_DATA_WANTS_ALL_NVEHS:
		*long_value = 1; /* must be set to 1 if data for more than 2 nearby vehicles per lane and upstream/downstream is to be passed from Vissim */
		return 1;
...
//}
//
//DRIVERMODEL_API void test2(long   type,
//						long   index1,
//						long   index2,
//						long   index3,
//						long long_value,
