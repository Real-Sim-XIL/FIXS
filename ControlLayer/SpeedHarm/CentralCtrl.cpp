...
void CentralCtrl::coordinate_ind(const INP_Struct* INP, const PAR_Struct* PAR, double t_0, OutStruct* out, double t_des)
{
	...
	//****************** New definition of tf :
	double rho_ldv = 1.5; // minimum time gap
	double rho_hdv = 2.5;
	...
	if (INP->no_veh != INP->leadVehNo) {
		if (INP->sGap < 200) {
			if (INP->Type_veh == PAR->TYPE_CAV_HGV)
				tf1 = INP->tfs_prc + rho_hdv;
			else
				tf1 = INP->tfs_prc + rho_ldv;
		}
		else {// tf if there is not a preceding vehicle inside the control zone or the preceding is more than 100 m far
			//tf1 = t0s + (sf - s0s) / vf;
			tf1 = INP->t0_veh_arr_data + sf / vf;
		}
	}
	else { // Tf for the first vehicle
		tf1 = t0s + (sf - s0s) / vf;
	}
	tfs = max(tfs, tf1);
	...
	if (INP->no_veh.compare("1") == 0) {
		...
		tempLog << INP->no_veh.c_str() << "; " << t_0 << "; " << INP->Type_veh << "; " << v_des_set << "; " << INP->Pos_veh << "; "
			<< max(t_0 + t_set, tfs) << "; " << tfs << "; " << tfs_fix << "; "
			<< tfs_up << "; " << v0s << "; " << s0s << "; " << vf << "; " << sf << "; "
			<< tfs_th << "; " << tf1 << "; " << INP->leadVehNo << "; " << INP->tfs_prv_veh_arr << "; " << INP->tfs_prc << endl;
		tempLog.close();
	}
}
...
void CentralCtrl::runtimeStepCall(double simTime, vector <string> egoIdVec, unordered_map <string, VehData_t>& VehDataLastStep_v) {
	...
	for (auto& it: VehDataLastStep_v) {
		...
		float posX = any_cast<float>(VehDataLastStep_v[it.first]["positionX"]);
		float posY = any_cast<float>(VehDataLastStep_v[it.first]["positionY"]);
		...
		for (int iC = 0; iC < PAR.CZ_en_coords_x.size(); iC++) {
		...
		if (ArrivalInfo.czIndexArr.find(id) != ArrivalInfo.czIndexArr.end()) {
			...
			if (ArrivalInfo.czIndexArr[id] == 1 && distOnCz < PAR.CZ_M_len[0]) {
				if (ENABLE_VERBOSE) {
					printf("\tvehicle %s on control zone\n", id.c_str());

					FILE* f = fopen(MasterLogName.c_str(), "a");
					fprintf(f, "\tvehicle %s on control zone\n", id.c_str());
					fclose(f);
				}
	...
	for (int iV = 0; iV < vehIdCzM.size(); iV++) {
		...
		if (Veh.ENABLE_CTRL && ENABLE_CAV_APP) {
			...
			VehDataLastStep_v[Veh.idStr]["accelerationDesired"] = (float)Veh.desiredAcceleration;
