...
%% Run Batch Scripts
system("runCoordMergeVISSIM.bat")
...
system([' matlab -nodesktop -nosplash -r ', ...
    '"', ...
    sprintf('startVissim(''coordMerge'', %f); ', stopTime+0.1), ...
    'quit force; " & '])
...
temp = load('coordMergeVISSIM_orig','VehicleOut');
...
if numel(Orig.spdDes.time) ~= numel(Mod.spdDes.time) || numel(Orig.spdDes.signals.values(:)) ~= numel(Mod.spdDes.signals.values(:))
    message = '\nCOORDMERGE VISSIM : ego speed trace vector size mismatch \n';
...
elseif max(abs(Orig.spdDes.time-Mod.spdDes.time))>1e-5 || max(abs(Orig.spdDes.signals.values(:)-Mod.spdDes.signals.values(:)))>1e-5
    message = '\nCOORDMERGE VISSIM : ego speed mismatch \n';
    writeLog(filename, message);
    checkFailed = 1;
...
%% quit matlab
if ~checkFailed
    message = '===> PASSED COORDMERGE: VISSIM test ===> \n';
    writeLog(filename, message)
